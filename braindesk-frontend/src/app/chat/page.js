  ...
  const handleSend = async (e, customMsg = null) => {
    ...
    if (selectedFiles.length > 0) {
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: `Uploading ${selectedFiles.length} file(s)... ⏳`,
        },
      ]);
      ...
      for (const file of selectedFiles) {
        ...
        try {
          ...
          setMessages((prev) => [
            ...prev.slice(0, -1),
            { role: "assistant", content: `❌ Failed to upload ${file.name}` },
          ]);
          setIsLoading(false);
          return;
        }
      ...
      messageToSend =
        `[System Context: User has uploaded files: ${uploadedNames.join(", ")}].\n` +
        messageToSend;
    }
    ...
    try {
      ...
      if (!currentSessionId) {
        ...
        const titleText = messageToSend.replace(
          /\[System Context:.*?\].\n/,
          "",
        );
        const sessionRes = await api.post("/chats/new", {
          title: titleText.substring(0, 25) || "Uploaded Document",
        });
        currentSessionId =
          sessionRes.data._id ||
          sessionRes.data.chatId ||
      ...
      if (res.data && res.data.messages) {
        ...
        let aiContent =
          res.data.response || res.data.message || "Response received";
        setMessages((prev) => [
          ...prev,
          { role: "assistant", content: aiContent, isNew: true },
        ]);
      }
    } catch (error) {
  ...
  const featureCards = [
    {
      ...
      instructions:
        "💡 Prompt: 'Create/Update a file in my repo' or 'Push this code'. Note: Cannot delete entire repositories for security.",
    },
    {
      icon: FlaskConical,
      title: "API Testing",
      ...
      instructions:
        "💡 Prompt: 'Test my login API'. It automatically deduces JSON schema, generates random emails, tests edge cases, and chains requests!",
    },
    {
      icon: Youtube,
      ...
      instructions:
        "💡 Prompt: 'Save this YouTube link' or upload a PDF. Then ask: 'Based on my context, what did the video say?'",
    },
  ];
      ...
      <aside className="w-64 bg-card border-r border-border flex flex-col justify-between hidden md:flex shrink-0 transition-colors duration-500">
        ...
        <div className="p-5 border-t border-border space-y-2">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-muted hover:bg-red-500/10 hover:text-red-500 transition-colors"
          >
      ...
      <main className="flex-1 flex flex-col relative min-w-0 bg-background transition-colors duration-500">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 pb-32 scroll-smooth custom-scrollbar">
          ...
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center pb-20">
              ...
              <h1 className="text-3xl font-bold mb-2">
                How can BrainDesk help today?
              </h1>
              <p className="text-muted text-sm max-w-md mb-8">
                Ask a question, upload a PDF, test an API, or manage GitHub.{" "}
                <br />
                <strong className="text-accent">
                  BrainDesk automatically routes your query to the right agent.
                </strong>
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl w-full">
                {featureCards.map((card, idx) => (
                  <div
                    key={idx}
                    className="relative group p-4 rounded-2xl bg-card border border-border hover:border-accent/50 transition-all text-left"
                  >
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="relative">
                        <Info
                          size={16}
                          className="text-muted hover:text-foreground cursor-pointer peer"
                        />
                        {/* Hover Popup */}
          ...
          <div className="space-y-6 pb-24 max-w-4xl mx-auto">
            {messages.map((msg, idx) => (
              ...
              >
                <div
                  className={`w-8 h-8 shrink-0 rounded-full flex items-center justify-center mt-1 shadow-sm ${msg.role === "user" ? "bg-accent" : "bg-card border border-border"}`}
                >
                  {msg.role === "user" ? (
                ...
                <div
                  className={`px-5 py-3.5 rounded-2xl max-w-[85%] text-[15px] leading-relaxed shadow-sm ${
                    msg.role === "user"
                      ...
                      : "bg-card border border-border text-foreground rounded-tl-sm ai-prose overflow-x-auto"
                  }`}
                >
        ...
        <div className="absolute bottom-0 w-full bg-gradient-to-t from-background via-background to-transparent pt-10 pb-4 px-4">
          <div className="max-w-4xl mx-auto">
            ...
            {selectedFiles.length > 0 && (
              <div className="flex gap-2 mb-2 px-2">
                {selectedFiles.map((file, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-2 bg-hover border border-border px-3 py-1.5 rounded-lg text-xs font-medium text-foreground"
                  >
            ...
            <form
              onSubmit={handleSend}
              className="flex items-end gap-2 bg-card border border-border rounded-xl p-2 shadow-lg focus-within:border-accent/50 focus-within:ring-1 focus-within:ring-accent/50 transition-all relative z-10"
            >
              {/* File Upload Button */}
              <input
                type="file"
            ...
            <div className="text-center mt-2 text-[11px] text-muted font-mono flex items-center justify-center gap-1">
              For best results, be specific. e.g., "Test my /login endpoint with
              edge cases."
            </div>
          </div>
        </div>
